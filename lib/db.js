...
export async function migrateConversationTablesForAnonymous() {
  try {
    ...
    logInfo('info', 'Migration conversations anonymes réussie');
    ...
    logInfo('error', 'Erreur migration conversations anonymes', { error: error.message });
    throw error;
  }
}
...
export async function createAnonymousConversationSession(anonymousId, ipAddress, userAgent, firstQuestion) {
  try {
    ...
    const sessionId = result.rows[0].id;
    logInfo('info', 'Session anonyme créée', { sessionId, anonymousId });

    return sessionId;
  } catch (error) {
    logInfo('error', 'Erreur création session anonyme', { error: error.message });
    throw error;
  }
}
...
export async function getAnonymousConversationSessions(limit = 50, offset = 0, dateFilter = null) {
  try {
    ...
    logInfo('error', 'Erreur récupération sessions anonymes', { error: error.message });
    throw error;
  }
}
...
export async function countAnonymousConversationSessions(dateFilter = null) {
  try {
    ...
    logInfo('error', 'Erreur comptage sessions anonymes', { error: error.message });
    throw error;
  }
}
