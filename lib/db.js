 ...
 */
export async function createOrder({ sessionId, userEmail, offerType, stripeCheckoutId = null }) {
  const AMOUNT_CENTS = { express_39: 3900, premium_99: 9900, avocat_gratuit: 0 };
  try {
    const result = await sql`
      INSERT INTO orders (session_id, user_email, offer_type, stripe_checkout_id, amount_cents)
      VALUES (${sessionId || null}, ${userEmail}, ${offerType}, ${stripeCheckoutId}, ${AMOUNT_CENTS[offerType] || 0})
      RETURNING id
    `;
    logger.info('Commande créée:', { orderId: result.rows[0].id, offerType, userEmail });
